...
const startCloudFloating = () => {
  $(".cloud").each(function (index, cloud) {
    leftValue = generateRandomInt(
      index * perCloudWidth,
      (index + 1) * perCloudWidth - cloudWidth
    );
...
const setMorningScene = () => {
  ...
  $("#sun").css({
    left: `calc(100% - ${sunHeightWidth / 2}px)`,
    top: `calc(100% - ${sunHeightWidth / 2}px)`,
  });
};
...
const setAfternoonScene = () => {
  ...
  $("#sun").css({
    left: `calc(50% - ${sunHeightWidth / 2}px)`,
    top: `${getTopValueOfSun(radius + "px")}px`,
  });
};
...
const enableDrag = (element) => {
  ...
  function elementDrag(e) {
    ...
    if (
      e.clientX <=
        currentWindowWidth *
          ((2 * radiusInPercentage + marginSpaceAtEachSideOfSunInPercentage) /
            100) &&
      e.clientX >=
        currentWindowWidth * (marginSpaceAtEachSideOfSunInPercentage / 100)
    ) {
      pos1 = pos3 - e.clientX;
      pos2 = pos4 - e.clientY;
      pos3 = e.clientX;
      ...
      const containerWidth =
        (currentWindowWidth * radiusInPercentage * 2) / 100;
      const eveningPer = 0.1;
      const morningPer = 0.1;
      const noonPer = 0.8;
      if (
        currentLeft >= containerWidth * eveningPer &&
        currentLeft <= containerWidth * (eveningPer + noonPer)
      ) {
...
const toggleDayNight = () => {
  ...
  if (isDay) {
    ...
    $(".night-back").css({ opacity: 0 });
  } else {
    $(".night-back").css({ opacity: 1 });
    $("#sun").addClass("moon");
  }
};
...
const getYCordinateOfCircle = (x) => {
  ...
  x =
    x +
    sunHeightWidth / 2 +
    (marginSpaceAtEachSideOfSunInPercentage * currentWindowWidth) / 100 -
    center.x;
